...
class SixBySixMatrixDesign extends StatefulWidget {
  final int randNum;
  final Function(int) onScoreChanged;
  const SixBySixMatrixDesign({
    Key? key,
  ...
  @override
  _SixBySixMatrixDesignState createState() => _SixBySixMatrixDesignState();
}

class _SixBySixMatrixDesignState extends State<SixBySixMatrixDesign> {
  String tempWord = '';
  int listLength = 0;
  List completedWords = [];
  int totalItem = WordList.listOfFiveByFiveWords.length;

  @override
  Widget build(BuildContext context) {
    List wordList = WordList.listOfSixBySixWords[widget.randNum];
    List matrixList = MatrixList.listOfSixBySixMatrix[widget.randNum];
                    ...
                    children: [
                      listLength >= 9
                          ? completedWords.contains(wordList[8])
                              ? CompletedWordDesign(word: wordList[8])
                              : NormalWordDesign(word: wordList[8])
                          : const SizedBox(),
                      const SizedBox(width: 5),
                      listLength >= 10
                          ? completedWords.contains(wordList[9])
                              ? CompletedWordDesign(word: wordList[9])
                              : NormalWordDesign(word: wordList[9])
                          : const SizedBox(),
                      const SizedBox(width: 5),
                      listLength >= 11
                          ? completedWords.contains(wordList[10])
                              ? CompletedWordDesign(word: wordList[10])
                              : NormalWordDesign(word: wordList[10])
                          : const SizedBox(),
                      const SizedBox(width: 5),
                      listLength >= 12
                          ? completedWords.contains(wordList[11])
                              ? CompletedWordDesign(word: wordList[11])
                              : NormalWordDesign(word: wordList[11])
                          : const SizedBox(),
                    ],
                  ),
                ],
  ...
  Widget getWordDesign({required String word}) {
    return Container(
      height: 40,
      width: 98,
      decoration: BoxDecoration(
